...
function BindValidation() {
    ...
    $(".numberonlywithnegativevalues").off("keypress").on("keypress", function (evt) {
        return isNumberWithNegativeValues(evt, this);
    });
    $(".numberonlywithnegativevalues").off("change").on("change", function (evt) {
            ...
            if (data.IsFile) {
                ...
                AlertModal('Success', ParseMessage(data.Messages), false, function () {
                    if (window.location.href.indexOf('&id=' + data.ItemID + "&") >= 0) {
                        ...
                        ShowWaitDialog();
                        window.location = window.location.href.replace("&id={ItemId}&", "&id=" + data.ItemID + "&").replace("&id=", "&id=" + data.ItemID + "&").replace("&urlAction=", "&urlAction=&");
                    }
                });
            }
        }
...
function Print(ele) {
    ...
    frameDoc.document.write("<html><head><style type='text/css'>html{height:0px !important; } .table{font-size:13px !important; padding:1px !important;}" +
        ".table tr td{padding:1px !important; text-align:center} .table thead td{font-weight:bold;}" +
...
function isNumberWithNegativeValues(evt, control)
{
    var charCode = (evt.which) ? evt.which : evt.keyCode;
    if (charCode == 37 || charCode == 38 || charCode == 39 || charCode == 40 || charCode == 35 || charCode == 36)
        return true;
...
function adjustFrameSize(contentHeight) {
    ...
    if (args.length < 2 && document.referrer) {
        ...
        params =args[1] !=undefined ?  args[1].split("&"):'';
    }
