        ...
        private UserDetails SPCurrentUser
        {
            get
            {
                ...
                else
                {
                    ...
                    Logger.Info("Get SPCacheKey as Session Key " + aCookie.Value);
            ...
            set
            {
                ...
                else
                {
                    var spContext = SharePointContextProvider.Current.GetSharePointContext(HttpContext);
                    HttpCookie aCookie = Request.Cookies["SPCacheKey"];
                    Logger.Info("Set SPCacheKey as Session Key " + aCookie.Value);
                    if (!string.IsNullOrEmpty(aCookie.Value))
                    {
            ...
            get
            {
                if (this.SPCurrentUser == null)
                {
                    ...
                    else
                    {
                        try
                        {
                            Logger.Info("enter into Get_CurrnetUser");
                            User spUser = null;
                            var spContext = SharePointContextProvider.Current.GetSharePointContext(HttpContext);
